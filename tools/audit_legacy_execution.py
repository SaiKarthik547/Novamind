    ...
    for root, _, files in os.walk(directory):
        ...
        for file in files:
            ...
            if "subprocess" in content and not any(allowed in filepath for allowed in ["core\\execution", "core/execution", "core\\legacy", "core/legacy", "tools", "core\\adapters", "core/adapters"]):
                violations.append(f"{filepath}: Imports 'subprocess'. All subprocess execution MUST route through KernelExecutionFacade via 'shell.execute'.")
            ...
            if "UNSAFE_RUNTIME" in content and not ("capability_registry.py" in filepath or "kernel_facade.py" in filepath or "test_" in filepath):
                violations.append(f"{filepath}: Claims 'UNSAFE_RUNTIME' authority. This authority level is strictly quarantined.")
    ...
    if args.generate_baseline:
        with open(args.generate_baseline, "w") as f:
            for v in violations:
                ...
                rel_path = os.path.relpath(v.split(":")[0], core_dir).replace("\\", "/")
                reason = v.split(":", 1)[1]
